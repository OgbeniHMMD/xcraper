(() => {
  // Guard against being injected more than once into the same tab/page.
  if (window.__xcrapperLoaded) return;
  window.__xcrapperLoaded = true;

  let isScraping = false; // Per-tab scraping flag
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "start_scraping") {
      ...
      sendResponse({ status: "started", isScraping });
      return true;
    }
    ...
    if (request.action === "stop_scraping") {
      ...
      sendResponse({ status: "stopped", isScraping });
      return true;
    }
  ...
  async function startPersistentCollection() {
    ...
    const grabAndSave = async () => {
      ...
      articles.forEach((article) => {
        ...
        const videoContainer = article.querySelector('div[data-testid="videoPlayer"], video, [aria-label="Embedded video"]');
    ...
    // Notify background script that scraping stopped
